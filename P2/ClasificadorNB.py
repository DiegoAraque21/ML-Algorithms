...
class ClasificadorNaiveBayes(Clasificador):
    ...
    def entrenamiento(self, datostrain: pd.DataFrame, nominalAtributos: list, diccionario: dict):
        ...
        self.likelihoods = np.empty((len(nominalAtributos) - 1, nclasses), dtype=object)
        ...
        for i in range(len(nominalAtributos) - 1):
            ...
            if nominalAtributos[i] == True:
                ...
                for c in range(nclasses):
                    ...
                    for v in range(len(diccionario[colnames[i]])):
                        ...
                        num = datostrain.loc[(datostrain.iloc[:, -1] == c) & (datostrain.iloc[:, i] == v), colnames[i]].count()

                        # calculate the likelihood
                        self.likelihoods[i, c] = num / classcol.value_counts()[c]

                        if self.laplace == True:
                            self.likelihoods[i, c] = (num + 1) / (classcol.value_counts()[c] + len(diccionario[colnames[i]]))
            ...
            # if the attribute is numeric
                ...
                for c in range(nclasses):
                    ...
                    values_ic = datostrain.loc[datostrain.iloc[:, -1] == c, colnames[i]]
                    self.likelihoods[i, c] = (values_ic.mean(), values_ic.std())
    ...
    def clasifica(self, datostest: pd.DataFrame, nominalAtributos: list, diccionario: dict) -> np.ndarray:
        ...
        posterioris = np.empty((len(nominalAtributos) - 1, nclasses), dtype=object)
        ...
        for dat, row  in zip(testdata, range(n)):
            for i in range(len(nominalAtributos) - 1):
                if nominalAtributos[i] == True:
                    for c in range(nclasses):
                        posterioris[i, c] = self.likelihoods[i, c] * self.prioris[c]
                elif nominalAtributos[i] == False:
                    for c in range(nclasses):
                        posterioris[i, c] = norm.pdf(dat[i], self.likelihoods[i, c][0], self.likelihoods[i, c][1]) * self.prioris[c]
